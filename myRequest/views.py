...
class UserObjectMixins(object):
    ...
    O_DATA_AUTH = aiohttp.BasicAuth(
        config.WEB_SERVICE_UID, config.WEB_SERVICE_PWD)
    ...
    def make_soap_request(self, soap_headers, endpoint, *params):
        ...
        if not session:
            ...
            session.auth = HTTPBasicAuth(
                soap_headers['username'], soap_headers['password'])
        with ThreadPoolExecutor() as executor:
            client = Client(config.BASE_URL,
                            transport=Transport(session=session))
            response = executor.submit(
                client.service[endpoint], *params).result()
        return response
    ...
    def quotes(self):
        ...
        try:
            response = requests.get(
                api_url, headers={'X-Api-Key': QUOTES_API_KEY})
